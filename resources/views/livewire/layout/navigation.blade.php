...
<div x-data="{ sidebarOpen: false }">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center justify-start rtl:justify-end">
                    ...
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="inline-flex items-center p-2 text-sm text-gray-100 rounded-lg min-[836px]:hidden hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-200">
    ...
    <!-- SIDEBAR -->
    <aside
        class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform bg-white border-r border-gray-200 shadow-lg shadow-gray-300/80 
           transform -translate-x-full min-[836px]:translate-x-0"
        :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full min-[836px]:translate-x-0'" aria-label="Sidebar">
                    ...
                    <li x-data="{ open: {{ collect($link['submenu'] ?? [])->contains(fn($s) => $s['active']) ? 'true' : 'false' }} }">
                        @if (isset($link['submenu']))
                            ...
                            <button @click="open = !open"
                                class="flex items-center justify-between w-full p-2 rounded-lg transition-all duration-200
                                text-gray-600 hover:bg-gray-100 hover:text-gray-900
                                {{ collect($link['submenu'])->contains(fn($s) => $s['active']) ? 'bg-blue-50 text-blue-700 font-semibold' : '' }}">
                        ...
                        @else
                            <!-- Opción normal -->
                            <a href="{{ $link['url'] ?? '#' }}"
                                class="flex items-center w-full p-2 rounded-lg transition-all duration-200
                                {{ $link['active']
                                    ? 'bg-[#21388B] text-white shadow-sm'
                                    : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}">
                                <i
    ...
    <div x-show="sidebarOpen" x-cloak @click="sidebarOpen = false"
        class="fixed inset-0 bg-black bg-opacity-50 z-30 max-[835px]:block min-[836px]:hidden">
    </div>
